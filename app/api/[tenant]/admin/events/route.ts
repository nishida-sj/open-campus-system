  ...
  try {
    ...
    const eventsWithApplicants = await Promise.all(
      (events || []).map(async (event) => {
        ...
        return { ...event, total_applicants: count || 0 };
      })
    );
...
export async function POST(
  ...
  try {
    ...
    if (!name || !dates || dates.length === 0) {
      return NextResponse.json({ error: 'イベント名と開催日程を入力してください' }, { status: 400 });
    }
    ...
    for (const date of dates) {
      if (!date.date || !date.capacity || date.capacity < 1) {
        return NextResponse.json({ error: '開催日と定員を正しく入力してください' }, { status: 400 });
      }
    }
    ...
    if (courses && courses.length > 0) {
      for (const course of courses) {
        if (!course.name || !course.name.trim()) {
          return NextResponse.json({ error: 'コース名を入力してください' }, { status: 400 });
        }
        if (!course.applicable_date_indices || course.applicable_date_indices.length === 0) {
          return NextResponse.json({ error: `コース「${course.name}」に適用する日程を選択してください` }, { status: 400 });
        }
      }
    }
    ...
    if (datesError) {
      ...
      return NextResponse.json({ error: '開催日程の作成に失敗しました', details: datesError.message }, { status: 500 });
    }
      ...
      if (insertedCourses) {
        ...
        if (associations.length > 0) {
          ...
          if (associationsError) {
            ...
            await supabaseAdmin.from('open_campus_events').delete().eq('id', event.id);
            return NextResponse.json({ error: 'コースと日程の関連付けに失敗しました' }, { status: 500 });
          }
        }
        ...
        if (capacities.length > 0) {
          ...
          if (capacitiesError) {
            ...
            await supabaseAdmin.from('open_campus_events').delete().eq('id', event.id);
            return NextResponse.json({ error: 'コース定員の登録に失敗しました' }, { status: 500 });
          }
        }
