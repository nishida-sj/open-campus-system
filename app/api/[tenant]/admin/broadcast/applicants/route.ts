...
export async function POST(
  ...
  try {
    ...
    // 選択されたイベントの日程を取得（テナント内のイベントのみ）
    // まずイベントがこのテナントに属することを確認
    const { data: tenantEvents, error: tenantEventsError } = await supabaseAdmin
      .from('open_campus_events')
      .select('id')
      .eq('tenant_id', tenant.id)
      .in('id', event_ids);

    if (tenantEventsError) {
      console.error('イベント確認エラー:', tenantEventsError);
      return NextResponse.json({ error: 'イベントの確認に失敗しました' }, { status: 500 });
    }

    const validEventIds = (tenantEvents || []).map((e) => e.id);
    ...
    // 確定済み申込者のみを取得（statusがconfirmedまたはcompleted、テナント内のみ）
