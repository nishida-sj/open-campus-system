...
export default function UsersPage() {
  ...
  const fetchData = async () => {
    try {
      // ユーザー一覧取得
      const usersRes = await fetch(`/api/${tenant}/admin/users`);
      ...
      // ロール一覧取得
      const rolesRes = await fetch(`/api/${tenant}/admin/roles`);
  ...
  const handleUpdateUser = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/${tenant}/admin/users/${editingUser.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
