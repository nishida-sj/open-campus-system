...
export default function AdminEventsPage() {
  ...
  const handleDeleteEvent = async (eventId: string, eventName: string) => {
    ...
    try {
      const response = await fetch(`/api/${tenant}/admin/events/${eventId}`, {
        method: 'DELETE',
      });
  ...
  const copyApplyUrl = async (eventId: string) => {
    ...
    const applyUrl = `${baseUrl}/${tenant}/apply?event=${eventId}`;
                      ...
                      >
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                          ...
                          <div>
                            ...
                            />
                            <p className="text-xs text-gray-500 mt-1">
                              コースを作成すると、対象日のコース定員の合計が自動登録されます
                            </p>
                          </div>
                        </div>
                        <button
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm text-gray-600 truncate font-mono">
                              {typeof window !== 'undefined' ? `${window.location.origin}/${tenant}/apply?event=${event.id}` : `/${tenant}/apply?event=${event.id}`}
                            </p>
                          </div>
                          <button
                      ...
                      <div className="mt-4 flex items-center gap-3">
                        <button
                          onClick={() => router.push(`/${tenant}/admin/events/${event.id}/edit`)}
