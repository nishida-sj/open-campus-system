...
export default function BroadcastPage() {
        ...
        if (response.ok) {
          ...
          const eventsWithFiscalYear = await Promise.all(
            data.map(async (event: Event) => {
              try {
                const datesRes = await fetch(`/api/${tenant}/admin/events/${event.id}`);
                if (datesRes.ok) {
  ...
  useEffect(() => {
    const fetchApplicants = async () => {
      ...
      try {
        const response = await fetch(`/api/${tenant}/admin/broadcast/applicants`, {
          method: 'POST',
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch(`/api/${tenant}/admin/broadcast/send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          type: messageType,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => router.push(`/${tenant}/admin/broadcast/history`)}
