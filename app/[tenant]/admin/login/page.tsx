...
function LoginForm() {
  ...
  const redirectTo = searchParams.get('redirectTo') || `/${tenant}/admin/events`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signInError) {
        // ログイン失敗を記録
        await fetch(`/api/${tenant}/auth/log-login`, {
          method: 'POST',
      ...
      // ログイン成功を記録
      const logResponse = await fetch(`/api/${tenant}/auth/log-login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email,
