...
export default function EventListPage() {
    ...
    const fetchData = async () => {
      try {
        const [eventsRes, settingsRes] = await Promise.all([
          fetch(`/api/${tenant}/events/public`),
          fetch(`/api/${tenant}/site-settings`),
        ]);
  ...
  const handleEventClick = (eventId: string) => {
    router.push(`/${tenant}/apply?event=${eventId}`);
  };
              ...
              {events.map((event) => (
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    {event.overview && (
                      <div className="bg-[#f8f9fa] border-l-2 p-3 mb-4" style={{ borderColor: settings.primary_color }}>
                        <p className="text-sm text-gray-700 line-clamp-3">{event.overview}</p>
                      </div>
                    )}
                    <div className="space-y-3 mb-5">
                      <div>
                        <div className="flex items-center text-sm font-bold mb-2" style={{ color: settings.primary_color }}>
                          ...
                          {event.dates && event.dates.length > 0 ? (
                            event.dates.map((date) => (
                              <div key={date.id} className="text-sm text-gray-700">
                                {new Date(date.date).toLocaleDateString('ja-JP', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'short' })}
                              </div>
                            ))
                          ) : (
                      ...
                      {event.allow_multiple_dates && (
                        <div className="flex items-center text-sm px-3 py-2" style={{ backgroundColor: `${settings.primary_color}10`, color: settings.primary_color }}>
                          <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
