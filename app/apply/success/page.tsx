...
function SuccessContent() {
  ...
  const lineBotId = process.env.NEXT_PUBLIC_LINE_BOT_BASIC_ID || '@471ktpxz'; // フォールバック値を追加
            ...
            <div className="text-center space-y-4">
              {!lineBotId || lineBotId === '' ? (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <p className="text-red-800 font-semibold">設定エラー</p>
                ...
                <>
                  {/* モバイル用: URL Schemeで自動送信 */}
                  {isMobile && (
                    <div className="space-y-3">
                      <a
                        href={lineMessageUrl}
                        className="inline-block bg-green-500 hover:bg-green-600 text-white font-bold text-lg py-4 px-8 rounded-lg transition duration-200 shadow-lg hover:shadow-xl w-full"
                      >
            ...
            <div className="border-t pt-6">
              ...
              {isMobile ? (
                <ol className="space-y-3 text-sm text-gray-700">
                  ...
                  <li className="flex items-start">
                    <span className="flex-shrink-0 w-6 h-6 bg-green-600 text-white rounded-full flex items-center justify-center text-xs font-bold mr-3">
                      ✓
                    </span>
                    <span className="font-semibold text-green-700">申込完了！通知が届きます</span>
                  </li>
                </ol>
              ) : (
                /* PC用の従来手順 */
                <ol className="space-y-3 text-sm text-gray-700">
                  ...
                  <li className="flex items-start">
                    ...
                    <span className="font-semibold text-green-700">申込完了の通知が届きます</span>
                  </li>
                </ol>
              )}
            </div>

            {/* 申込番号（モバイル: 折りたたみ可能、PC: 常に表示） */}
